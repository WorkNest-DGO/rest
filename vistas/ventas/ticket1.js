...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const ventaParam = params.get('venta');
    const almacenado = localStorage.getItem('ticketData');
    const sinDatos = document.getElementById('sinDatos');
    if (!almacenado && !ventaParam) {
    ...
    if (ventaParam) {
        ...
        const divReimp = document.getElementById('divReimprimir');
        if (divReimp) divReimp.style.display = 'block';
        const btnReimprimir = document.getElementById('btnReimprimir');
        if (btnReimprimir){ btnReimprimir.addEventListener('click', () => { window.open('../../api/tickets/reimprime_ticket.php?venta_id=' + ventaIdNum); }); }
        try {
          const $rp = document.getElementById('regPropinas');
          if ($rp) {
            $rp.style.display = 'block';
            ['propinaEfectivoD','propinaTarjetaD','propinaChequeD'].forEach(id=>{ const el = document.getElementById(id); if (el) el.style.display='block'; });
            const en = (id)=>{ const el = document.getElementById(id); if (el){ el.disabled=false; el.readOnly=false; } };
            ['propinaEfectivo','propinaTarjeta','propinaCheque'].forEach(en);
          }
          // Intentar precargar montos de propina si el backend los expone
          try {
            const res = await fetch('../../api/ventas/detalle_venta.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ venta_id: ventaIdNum }) });
            const data = await res.json();
            if (data && data.success) {
              const setVal = (id,val)=>{ const el = document.getElementById(id); if (el && typeof val !== 'undefined' && val !== null) el.value = String(val); };
              setVal('propinaEfectivo', data.propina_efectivo ?? '');
              setVal('propinaCheque',  data.propina_cheque  ?? '');
              setVal('propinaTarjeta', data.propina_tarjeta ?? '');
              // Mostrar detalle del ticket en pantalla
              try {
                const obj = {
                  venta_id: ventaIdNum,
                  productos: Array.isArray(data.productos) ? data.productos.map(p=>({ nombre: p.nombre, cantidad: Number(p.cantidad)||0, precio_unitario: Number(p.precio_unitario)||0 })) : [],
                  mesa_nombre: data.mesa || '',
                  mesero_nombre: data.mesero || '',
                  tipo_entrega: data.tipo_entrega || 'N/A'
                };
                llenarTicket(obj);
                const imp = document.getElementById('imprimir');
        ...
        const totalPropinas = parseFloat(datos.propina_efectivo || 0) + parseFloat(datos.propina_cheque || 0) + parseFloat(datos.propina_tarjeta || 0);
        if (parseFloat(totalPropinas) > parseFloat(0.00)){
            ...
            // Mostrar y habilitar captura de propinas en reimpresión
            try {
              const $rp = document.getElementById('regPropinas');
              if ($rp) {
                $rp.style.display = 'block';
                ['propinaEfectivoD','propinaTarjetaD','propinaChequeD'].forEach(id=>{ const el = document.getElementById(id); if (el) el.style.display='block'; });
                const setEn = (id,val)=>{ const el = document.getElementById(id); if (!el) return; el.disabled=false; el.readOnly=false; if (typeof val!== 'undefined' && val !== null) el.value = String(val); };
                setEn('propinaEfectivo', datos.propina_efectivo ?? '');
                setEn('propinaTarjeta', datos.propina_tarjeta ?? '');
                setEn('propinaCheque',  datos.propina_cheque  ?? '');
              }
              // Mostrar detalle del ticket en pantalla con los datos almacenados
              try {
                llenarTicket(datos);
                const imp = document.getElementById('imprimir');
                if (imp) imp.style.display = 'block';
            ...
            // Liberar mesa en reimpresión (flujo existente)
    ...
    // Handler de impresión del modal
    const btnImprimir = document.getElementById('btnImprimir');
    if (btnImprimir) btnImprimir.addEventListener('click', imprimirTicket);
    // Handler de guardado de propinas (disponible también en reimpresión)
...
// }).then(r=>r.json()).then(d=>{ /* manejar respuesta */ });
  ...
  async function capturaPropinas() {
        ...
        const info2 = (()=>{ try { return JSON.parse(localStorage.getItem('ticketData')); } catch(_) { return null; } })();
        const ventaParam = new URLSearchParams(window.location.search).get('venta');
        const ventaId = info2?.venta_id || (ventaParam ? parseInt(ventaParam,10) : null);
        if (!ventaId) { alert('No hay venta asociada'); return; }
