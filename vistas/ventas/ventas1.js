...
// Fallback mínimo si modal-lite/Bootstrap no está inicializado
if (typeof showModal !== 'function') {
    window.showModal = function(sel) {
...
// Si la modal no se ve, mostrar su texto en un alert (evita pantalla negra sin contenido)
(function(){
    const originalShow = window.showModal;
    window.showModal = function(sel) {
        if (originalShow) originalShow(sel);
        const el = typeof sel === 'string' ? document.querySelector(sel) : sel;
        if (!el) return;
        setTimeout(() => {
            const visible = el.classList.contains('show') || el.style.display === 'block';
            if (!visible) {
